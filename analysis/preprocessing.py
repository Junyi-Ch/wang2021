#This code was partly generated by Chatgpt-5
#Prompt: I uploaded my raw data files and asked it to clean them to only include time elapsed, mandarin prodiciency
# placements, and dissimilarity vectors.
"""
...
def extract_metadata(df):
    """
    Extracts participant-level metadata from the full raw dataframe:
      - total_time_sec (from time_elapsed, assumed ms)
      - mandarin_proficiency (from JSON in response)
    Returns (total_time_sec, mandarin_proficiency)
    """
    # 1) Completion time
    total_time_sec = None
    if "time_elapsed" in df.columns:
        try:
            # jsPsych time_elapsed is usually cumulative ms since start
            # Find first fullscreen trial
            start = df[df['trial_type'].str.contains('fullscreen', na=False)]['time_elapsed'].min()
            end = df['time_elapsed'].max()
            total_time_sec = (end - start) / 1000.0

        except Exception:
            total_time_sec = None

    # 2) Mandarin proficiency
    mandarin_proficiency = None
    age = None
    gender = None
    if "response" in df.columns:
        # loop through all non-null responses and try to parse JSON
        for resp_str in df["response"].dropna().astype(str):
            try:
                resp = json.loads(resp_str)
            except Exception:
                continue

            # we only care about JSON objects (dicts)
            if not isinstance(resp, dict):
                continue

            # Try a few possible key names; adjust as needed
            for key in ["mandarin_proficiency"]:
                if key in resp:
                    mandarin_proficiency = resp[key]
    ...
    return total_time_sec, mandarin_proficiency, age, gender
...
def clean_file(path):
    ...
    # ---- extract participant-level metadata BEFORE filtering ----
    total_time_sec, mandarin_proficiency, age, gender = extract_metadata(df)
    ...
    # ---- add metadata columns to cleaned data ----
    df_arr["time_elapsed_sec"] = total_time_sec
    df_arr["mandarin_proficiency"] = mandarin_proficiency
    df_arr["age"] = age
    df_arr["gender"] = gender
